    ...
    <section>
        <div class="body">
            ...
            <register inline-template>
                <form id="register" action="{{ url()->current() }}" method="post" autocomplete="off">
                    @csrf
                    ...
                    <div class="form-group">
                        <div class="form-table">
                            ...
                            <div class="tr">
                                ...
                                <div class="td">
                                    @if(count($roles))
                                        <select name="role" id="role" @change="role = $event.target.value">
                                            <option disabled selected>Maak een keuze</option>
                                            @foreach($roles as $role)
                                                <option
                                                    value="{{ $role }}" {{ $role === old('role') ? 'selected' : null }}>{{ $role }}</option>
                                            @endforeach
                                        </select>
                                    @else
                                    ...
                                    @if(count($practitioners))
                                        <select name="practitioner" id="practitioner">
                                            @foreach($practitioners as $name)
                                                <option
                                                    value="{{ $name }}" {{ $name === old('practitioner')? 'selected' : null }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    @else
                                    ...
                                    @if(count($groups))
                                        <select name="group" id="group">
                                            ...
                                            @foreach($groups as $name)
                                                <option
                                                    value="{{ $name }}" {{ $name === old('group')? 'selected' : null }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    @else
                    ...
                    <div class="form-group"
                         v-if="role && ['Ziekenhuis', 'Verzekering', 'Verpleegkundige'].indexOf(role) === -1">
                        <div class="form-table">
